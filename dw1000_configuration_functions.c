...
/*
 * Function which checks the SPI communication
 * The register 0x00 (DEV_ID) should always
 * have the value 0xDECA0130
 */
void check_spi()
{
    uint32_t dev_id;
...
void initialise()
{
    // Reset PMSC - reset internal clocks
    dw1000_write_u8(PMSC, 0x01);
    dw1000_subwrite_u8(PMSC, 0x01, 0x00);
    dw1000_subwrite_u16(PMSC, 0x04, 0x00);

    // Soft reset before starting initialisation
    ...
    // Set system clock to XTI for valid OTP reads
    dw1000_write_u8(PMSC, 0x01);
    dw1000_subwrite_u8(PMSC, 0x1, 0x00);

    // Configure CPLL lock detect
    ...
    dw1000_write_u8(PMSC, 0x01);
    dw1000_subwrite_u8(PMSC, 0x01, 0x03);

    // Load LDE microcode
    dw1000_subwrite_u16(OTP_IF, OTP_CTRL, 0x8000);

    dw1000_write_u8(PMSC, 0x00);
    dw1000_subwrite_u8(PMSC, 0x01, 0x00);
    dw1000_write_u8(PMSC, 0x00);
    dw1000_subwrite_u8(PMSC, 0x01, 0x00);
...
void configure()
{
    ...
    dw1000_subwrite_u32(AGC_CFG_STS, 0xC, 0x2502A907);
    dw1000_subwrite_u32(AGC_CFG_STS, 0x4, 0x0000889B);
    ...
    dw1000_subwrite_u32(PMSC, 0x00, 0xF0300100);

    dw1000_subwrite_u32(PMSC, 0x28, 0x00000110);
...
void rx_enable()
{
    uint16_t temp = (uint16_t)SYS_CTRL_RXENAB;
